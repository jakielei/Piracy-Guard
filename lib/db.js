...
function addSearchResults(dramaDbId, results) {
  ...
  const insert = db.prepare(
    'INSERT INTO search_results (drama_db_id, url, title, snippet, page_number, domain, match_status, match_reason, is_pirated) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)'
  );

  db.transaction(() => {
    for (const r of results) {
      const domain = extractDomain(r.url);
      insert.run(dramaDbId, r.url, r.title, r.snippet, r.pageNumber, domain, r.match_status || 'unknown', r.match_reason || '', r.match_status === 'piracy' ? 1 : null);
    }
  })();
}
