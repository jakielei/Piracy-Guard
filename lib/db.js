...
function initSchema() {
  ...
  try { db.exec(`ALTER TABLE search_results ADD COLUMN match_status TEXT DEFAULT 'unknown'`); } catch(e) {}
  try { db.exec(`ALTER TABLE search_results ADD COLUMN match_reason TEXT`); } catch(e) {}
  try { db.exec(`ALTER TABLE search_results ADD COLUMN site_name TEXT`); } catch(e) {}
  try { db.exec(`ALTER TABLE search_results ADD COLUMN display_url TEXT`); } catch(e) {}
  try { db.exec(`ALTER TABLE tasks ADD COLUMN name TEXT DEFAULT ''`); } catch(e) {}
}
...
function createTask(dramaList) {
  ...
  // Generate task name: "X月X日 第N次"
  const now = new Date();
  const month = now.getMonth() + 1;
  const day = now.getDate();
  const todayStr = `${now.getFullYear()}-${String(month).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
  const todayCount = db.prepare(
    "SELECT COUNT(*) as cnt FROM tasks WHERE date(created_at) = ?"
  ).get(todayStr).cnt;
  const taskName = `${month}月${day}日 第${todayCount + 1}次`;

  const insertTask = db.prepare(
    'INSERT INTO tasks (total_dramas, name) VALUES (?, ?)'
  );
  ...
  const result = db.transaction(() => {
    const taskResult = insertTask.run(dramaList.length, taskName);
...
function addSearchResults(dramaDbId, results) {
  ...
  const insert = db.prepare(
    'INSERT INTO search_results (drama_db_id, url, title, snippet, page_number, domain, match_status, match_reason, is_pirated, site_name, display_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
  );

  db.transaction(() => {
    for (const r of results) {
      const domain = extractDomain(r.url);
      insert.run(dramaDbId, r.url, r.title, r.snippet, r.pageNumber, domain, r.match_status || 'unknown', r.match_reason || '', r.match_status === 'piracy' ? 1 : null, r.siteName || '', r.displayUrl || '');
    }
  })();
}
