...
async function matchResult(result, dramaName, browserContext, fastMode = false) {
  ...
  // Rule 3: Domain whitelist — known safe/non-piracy domains
  const safeDomains = [
    'dramaboxdb.com',
    'imdb.com',
    'wikipedia.org',
    'spotify.com',
    'iq.com',
    'amazon.',       // matches amazon.com, amazon.co.jp, amazon.de, etc.
    'dramasnacker.com',
    'dramaboxapp.com',
    'mydramalist.com',
    'koreanallday.com',
  ...
  if (isSafeDomain) {
    return { match_status: 'safe', match_reason: `已知安全域名 (${domain})，自动判定安全` };
  }
...
function matchSocialMedia(result, domain) {
  ...
  if (accountName) {
    // Step 3a: Check if account name contains known official brand names
    const accountLower = accountName.toLowerCase().trim();
    const officialBrands = ['dramabox', 'reelshort'];
    for (const brand of officialBrands) {
      if (accountLower.includes(brand)) {
        return { match_status: 'safe', match_reason: `社媒账号包含官方品牌名 "${brand}": ${accountName}，直接判定安全` };
      }
    }

    // Step 3b: Compare account name against whitelist (case-insensitive)
